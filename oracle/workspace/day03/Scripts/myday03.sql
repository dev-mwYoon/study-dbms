...
/*PLAYER 테이블에서 TEAM_ID가 'K03' 이거나 'K07'이고 포지션은 'MF'인 선수*/
SELECT *
FROM PLAYER p 
WHERE (TEAM_ID = 'K03' OR TEAM_ID = 'K07') AND "POSITION" = 'MF';

SELECT *
FROM PLAYER p 
WHERE TEAM_ID IN ('K03', 'K07') AND "POSITION" = 'MF';
...
/*PLAYER 테이블에서 NICKNAME이 NULL인 선수 검색*/
 SELECT PLAYER_NAME , NICKNAME  FROM PLAYER p 
...
/*띄어쓰기는 쌍따옴표로 묶어서 작성*/
SELECT PLAYER_NAME , NVL("POSITION", '미정') "포지션 여부"
FROM PLAYER p ;


/*선수 이름과 생일 조회*/
SELECT PLAYER_NAME "선수 이름",BIRTH_DATE "선수 생일"
FROM PLAYER p ;
/*PLAYER 테이블에서 BACK_NO를 "등 번호"로, NICKNAME을 "선수 별명"으로 변경하여 검색*/
SELECT BACK_NO AS "등 번호", NICKNAME AS "선수 별명"
FROM PLAYER p ;
...
/*PLAYE_NAME의 별명은 NICKNAME이다.*/
SELECT PLAYER_NAME || '의 별명은 ' || NICKNAME || '이다.' AS 자기소개
FROM PLAYER p ;

/*PLAYER_NAME의 영어이름은 E_PLAYER_NAME이다.*/
SELECT PLAYER_NAME || '의 영어이름은 ' || E_PLAYER_NAME || '이다.' AS "영어이름 소개"
FROM PLAYER p ;


/*PLAYER_NAME의 포지션은 POSITION입니다.*/
SELECT PLAYER_NAME || '의 포지션은 ' || NVL("POSITION", '미정')  || '입니다.' AS "포지션 소개"
FROM PLAYER p ;
 ...
 * 'A%' : A로 시작하는 모든 값
 * '%A%' : A가 포함된 모든 값
 * 'A___' : A로 시작하는 3(언더바 개수)글자인 값
 * '__A' : A로 끝나고 2(언더바 개수)글자인 값
 * 
 */
SELECT * FROM TEAM t2 ;
...
/* 표준 : WHERE 뒤에 NOT을 붙이는 것*/
SELECT *
FROM PLAYER p 
WHERE NOT PLAYER_NAME LIKE '이%';

/* ORACLE : LIKE 앞에 NOT을 붙이는 것*/
SELECT *
FROM PLAYER p 
...
SELECT AVG(HEIGHT), MAX(HEIGHT), MIN(HEIGHT), SUM(HEIGHT), COUNT(HEIGHT)  
FROM PLAYER p ;

/*PLAYER 테이블에서 HEIGHT로 총 선수 명수 검색*/
SELECT COUNT(NVL(HEIGHT, 0)) "총 인원"
FROM PLAYER p ;
...
/*
 * SUB QUERY( 메인을 보조해주는 쿼리. 즉, 쿼리 안에 쿼리)
 * 
 * FROM절 : IN LINE VIEW
...
(
	SELECT AVG(HEIGHT) 
	FROM PLAYER p 
) AS "전체 평균 키" , AVG(HEIGHT) AS "포지션별 평균 키" 
FROM PLAYER p
WHERE HEIGHT IS NOT NULL 
GROUP BY "POSITION"; 
 ...
 * 정규화를 통해 조회 테이블이 너무 많이 쪼개져 잇으면 작업이 불편하기 때문에
 * 입력, 수정, 삭제의 성능을 향상시키기 위해서 JOIN을 통해 합친 후 사용한다.
 * 
 */

/*EMP 테이블 사원번호로 DEPT 테이블의 지역 검색. DEPT=부서*/
SELECT * FROM EMP e ;
SELECT * FROM DEPT;

/*
 * JOIN을 기준으로 앞에가 선행 테이블.
 * 선행테이블이 일이고 뒤에가 다가 되어야함.
 */
SELECT ENAME, LOC 
...
/*WHERE절은 FROM절이 모두 완성되어야 실행되기 때문에
 * ON절에 조건을 추가해서 불필요한 조건 검색을 제외한다.
 * */
SELECT T.TEAM_ID , P.PLAYER_ID , T.TEL 
FROM TEAM t JOIN PLAYER p
